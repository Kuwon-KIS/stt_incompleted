...
class BatchResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        d = asdict(self)
        # Serialize detected_issues to JSON string for storage
        try:
            d['detected_issues'] = json.dumps(self.detected_issues, ensure_ascii=False)
        except (TypeError, ValueError):
            # Fallback to empty list if serialization fails
            d['detected_issues'] = json.dumps([])
